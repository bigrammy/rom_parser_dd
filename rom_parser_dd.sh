...
# Additional output to generate .sh script for dd : Decker
# WWW: http://www.decker.su
#
# Usage:
# 1) python rom_parser_dd.sh ROM_0 > split.sh
# 2) . split.sh
#
# After launching split.sh ROM parts will be copied in split folder. 
...
def an_part_table(partition_table, gpt_header):
	...
	part_name = part_entry[5][::2].replace('\0',' ').rstrip()
	#print '  [-] Partition Name: "%s"' % part_name
	# print 'dd if=%s of=split/%s skip=%d count=%d ibs=%d' % (sys.argv[1], part_name,(part_entry[2] * LBA_SIZE),((part_entry[3]-part_entry[2]) * LBA_SIZE), 1)

	if (part_name == 'cache'):	
		print '# Making empty cache partition'
		print 'mkdir -p split/cache'
		print 'make_ext4fs -s -l %d -a cache split/cache.img split/cache/' % ((part_entry[3]-part_entry[2]) * LBA_SIZE)
	elif (part_name == 'userdata'):
		print '# Making empty userdata partition'
		print 'mkdir -p split/userdata'
		print 'make_ext4fs -s -l %d -a data split/userdata.img split/userdata/' % ((part_entry[3]-part_entry[2]) * LBA_SIZE)
	else:
		print 'dd if=%s of=split/%s skip=%d count=%d bs=%d status=progress' % (sys.argv[1], part_rename(part_name) ,(part_entry[2]),((part_entry[3]-part_entry[2]+1)), LBA_SIZE)
        count += 1
...
def usage(argv):
    print '\033[1;30mROM Parser v0.01alpha\033[0m (c) n0fate, \033[1;30mDecker\033[0m'
    print 'Usage (2 steps): '	
    print '	\033[1;30m1)\033[0m python %s <ROM_0> > split.sh'%argv[0] # python rom_parser_dd.sh ROM_0 > split.sh
    print '	\033[1;30m2)\033[0m . split.sh'	
    sys.exit()
